...
func CleanDatabase(client *notionapi.Client, databaseID string) error {
	startCursor := ""
	for {
		...
		resp, err := client.QueryDatabase(context.Background(), databaseID, &query)
		if err != nil {
			return fmt.Errorf("error querying database: %w", err)
		}

		for _, page := range resp.Results {
			// Accede al valor del checkbox "Guardar"
			...
			checkbox := false
			if cb, ok := prop.(*notionapi.CheckboxProperty); ok {
				checkbox = cb.Checkbox
			}

			if !checkbox {
				// Archiva la página (equivalente a borrarla en Notion)
				_, err := client.UpdatePage(context.Background(), page.ID, &notionapi.PageUpdate{
					Archived: true,
				})
				if err != nil {
					log.Printf("Error archiving page %s: %v", page.ID, err)
				}
			}
		}
